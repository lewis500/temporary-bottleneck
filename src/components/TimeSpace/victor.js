 ...
 */
function createFlowOfDensity({ maxDensity, peakDensity, peakFlow }) {
...
if (INVARIANT_CHECKS) {
  ...
  if (Math.abs(flowOfDensity(peakDensity) - peakFlow) > 0.000001) {
    ...
    throw new Error(
      `peak density should equal peak flow instead got ${flowOfDensity(
        peakDensity
      )}`
    );
  }
...
function getInterfaces(
  ...
  const ray = [start, [start[0] + 1, start[1] + velocity]];
  ...
  events.forEach(event => {
    ...
    const { closestInterface } = getClosestSegmentAboveInterface(
      start,
      interfaces
    );
  ...
  if (INVARIANT_CHECKS) {
    for (let i = 0; i < state.vehicles.length - 1; i++) {
      ...
      if (next.position < current.position) {
        ...
        console.error(next);
        throw new Error(
          `Order invariant failed current: ${current.id} next:  ${next.id}`
        );
      }
    }
  }
  ...
  return {
    ...
    vehicles: [
      ...state.vehicles.map((vehicle, index) => {
        ...
        if (status === "BLOCKED" && blockedByEvent !== undefined) {
          ...
          for (let event of activeEvents) {
            if (
              ...
              position + velocity * dt >= event.position
            ) {
              return {
                ...vehicle,
                position,
        ...
        let spacing = nextVehicle
          ? nextVehicle.position - vehicle.position
          : 10000;
...
class App extends React.Component {
  ...
  canvasRedraw() {
    ...
    context.beginPath();
    context.moveTo(
      this.scaleX(TIME_BOUNDS[0]),
      this.scaleY(POSITION_BOUNDS[0])
    );
    context.lineTo(
      this.scaleX(TIME_BOUNDS[1]),
      this.scaleY(POSITION_BOUNDS[0])
    );
    context.lineTo(
      this.scaleX(TIME_BOUNDS[1]),
      this.scaleY(POSITION_BOUNDS[1])
    );
    context.lineTo(
      this.scaleX(TIME_BOUNDS[0]),
      this.scaleY(POSITION_BOUNDS[1])
    ...
    const startVehiclePosition =
      history[0].vehicles[history[0].vehicles.length - 1].position;
  ...
  render() {
    ...
    return (
      <div>
        ...
        {SHOW_DEBUG ? (
          <div style={{ position: "absolute", left: width / 2, top: 0 }}>
            <code>
